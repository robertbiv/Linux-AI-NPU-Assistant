...
def test_query_battery(m_path, m_read, m_scan, tmp_path):
    ...
    m_scan.return_value = __import__("contextlib").nullcontext([DummyDirEntry("BAT0", str(bat))])

    def side_effect(path):
        if "type" in path: return "Battery"
        if "capacity" in path: return "80"
        if "status" in path: return "Discharging"
        return ""
...
@patch.dict(_QUERIES, {"cpu": lambda: "cpu info", "memory": lambda: "mem info"}, clear=True)
def test_query_all():
    res = _query_all()
